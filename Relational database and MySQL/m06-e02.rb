...
class MySQLExercises2
  ...
  def q_categories_with_parent
    ...
    <<-SQL
      SELECT DISTINCT parent.name as parent_name, categories.name as category_name
      FROM categories as parent JOIN categories
      ON parent.id = categories.parent_category_id
  ...
  def q_avg_stars_per_user
    ...
    <<-SQL
      SELECT reviews.user_id as id, users.name, AVG(stars) as avg_stars
      FROM reviews JOIN users
      ON reviews.user_id = users.id
      GROUP BY user_id
      HAVING avg_stars > 4
      ORDER BY avg_stars DESC, users.name
    SQL
  end
  ...
  def q_buyers_that_are_also_sellers
    ...
    <<-SQL
      ...
      FROM sales
      WHERE seller_id IN (SELECT buyer_id FROM sales)
